...
// gitlabJoin carries the GitLab work-board card identity that triggered a
// daemon prompt. The bridge sends it as HTTP headers (X-AgentOps-GitLab-*) on
// the daemon /prompt call; we stash it in the request context so that any
// AgentRun created via delegation during that prompt (run_agent/run_agents) is
// stamped with the agentops.dev/gitlab-* annotations the console uses to join
// runs to work-board cards. This makes the link deterministic and requires no
// cooperation from the LLM (no delegation tool param, no prompt instructions).
type gitlabJoin struct {
	IID     string
	Project string
	Target  string
}

type gitlabJoinContextKey struct{}

// withGitlabJoin returns ctx carrying the given GitLab card identity, but only
// when an iid is present (the discriminating field). A no-op otherwise.
func withGitlabJoin(ctx context.Context, j gitlabJoin) context.Context {
	if j.IID == "" {
		return ctx
	}
	return context.WithValue(ctx, gitlabJoinContextKey{}, j)
}

// gitlabJoinFromContext returns the GitLab card identity stashed by the prompt
// handler, if any.
func gitlabJoinFromContext(ctx context.Context) (gitlabJoin, bool) {
	j, ok := ctx.Value(gitlabJoinContextKey{}).(gitlabJoin)
	return j, ok
}
...
type AgentRunGitParams struct {
	...
	// Project is the full project path inside a gitlab-group ResourceRef
	// (e.g. "samyn92-lab/billing-svc"). Empty for project/github/git refs.
...
func (k *K8sClient) CreateAgentRun(ctx context.Context, agentRef, prompt, source, sourceRef, traceparent string, gitParams *AgentRunGitParams, extraLabels map[string]string) (*AgentRunResult, error) {
	...
	if gitParams != nil {
		// NOTE: the CRD field is spec.git.integrationRef (Required). It must
		// match what the operator/controller reads — writing "resourceRef"
		// here would be silently dropped and fail CRD validation.
		gitMap := map[string]interface{}{
			"integrationRef": gitParams.ResourceRef,
			"branch":         gitParams.Branch,
		}
		if gitParams.BaseBranch != "" {
			gitMap["baseBranch"] = gitParams.BaseBranch
		}
		if gitParams.Project != "" {
			gitMap["project"] = gitParams.Project
		}
		spec["git"] = gitMap
	}
	...
	// Propagate the GitLab work-board join (if this run was created while
	// servicing a bridge-triggered prompt). These annotations let the console
	// join the delegated run back to its work-board card — the same keys the
	// gitlab-label bridge stamps on the task-agent runs it creates directly.
	if j, ok := gitlabJoinFromContext(ctx); ok {
		annotations["agentops.dev/gitlab-iid"] = j.IID
		if j.Project != "" {
			annotations["agentops.dev/gitlab-project"] = j.Project
		}
		if j.Target != "" {
			annotations["agentops.dev/gitlab-target"] = j.Target
		}
	}
