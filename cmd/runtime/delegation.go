...
type DelegationInput struct {
	...
	GitProject    string `json:"git_project,omitempty" description:"For a gitlab-group git_resource: the full project path inside the group to clone (e.g. group/repo). Omit for project/github/git resources."`
}
...
func buildRunAgentsDescription(resources []ResourceEntry) string {
	...
	for _, r := range resources {
		...
		case "gitlab-group":
			if r.GitLabGroup != nil {
				detail := fmt.Sprintf("  - %q (GitLab group: %s — also set git_project to the full project path inside the group, e.g. %s/<repo>)",
					r.Name, r.GitLabGroup.Group, r.GitLabGroup.Group)
				if r.Description != "" {
					detail += " — " + r.Description
				}
				gitResources = append(gitResources, detail)
	...
	if len(gitResources) > 0 {
		base += "\n\nFor coding/git tasks, set git_resource + git_branch per delegation " +
			"(for a gitlab-group resource also set git_project to the project path inside the group). " +
...
func (dw *DelegationWatcher) processWatchEvents(ctx context.Context, w watch.Interface, groupID, runName string) {
	...
	// Poll fallback: a K8s watch can silently stall (idle connection dropped
	// half-open, no events + no close), which would leave a delegated run
	// undetected for a very long time and the parent agent stuck "busy". Poll
	// the run's status on a ticker so a terminal phase is detected within the
	// interval even if the watch delivers nothing.
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-ticker.C:
			status, err := dw.k8s.GetAgentRun(ctx, runName)
			if err != nil || status == nil || !isTerminalPhase(status.Phase) {
				continue
			}
			slog.Info("delegation run completed (poll fallback)",
				"groupId", groupID, "run", runName, "phase", status.Phase)
			dw.recordCompletion(groupID, runName, &RunResult{
				AgentName: agentNameFromRun(runName),
				Phase:     status.Phase,
...
// agentNameFromRun derives the agent name from a run name of the form
// "<agent>-run-<ts>-<hex>" (best-effort; used only for logging/summaries when a
// polled status doesn't include spec.agentRef).
func agentNameFromRun(runName string) string {
	if i := strings.Index(runName, "-run-"); i > 0 {
		return runName[:i]
	}
	return runName
}
